...
class test_argo_workflow_service(unittest.TestCase):
    @classmethod
    def get_workflow(cls):
        # 1. 准备 Mock 数据
        # 建议从 Argo 真实环境复制一份 JSON 存为文件，这里演示手动构造
        with open("dingtalk/services/mock/mock_workflow_data.json", "r", encoding="utf-8") as f:
            mock_json = json.load(f)

        # 将 JSON 转换为 Hera 的 Workflow 对象
        mock_workflow_obj = Workflow(**mock_json)
        ...
        with patch('hera.workflows.service.WorkflowsService.get_workflow') as mock_method:
            ...
            result = service.get_result("argo-workflows", "test-name")

            # 4. 验证逻辑
            print(f"解析后的状态: {result.status}")
            assert result.namespace == "argo-workflows"
...
#     unittest.main()
if __name__ == '__main__':
    test_argo_workflow_service.get_workflow()
